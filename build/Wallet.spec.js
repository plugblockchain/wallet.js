...
const TESTING_PAIRS = (0, _testingPairs.default)();
...
describe('a wallet', () => {
  ...
  it('create and restore', async () => {
    ...
    const keyring = new _HDKeyring.HDKeyring({
      mnemonic,
  ...
  describe('accounts', () => {
    it('addKeyring(keyring)', async () => {
      const keyring = new _SimpleKeyring.SimpleKeyring();
    ...
    it('throw error when trying to add keyring with duplicate accounts', async () => {
      const keyring = new _SimpleKeyring.SimpleKeyring();
      const keyring2 = new _SimpleKeyring.SimpleKeyring();
    ...
    it('try resolve conflicts #1', async () => {
      ...
      const vault = '';
      const wallet = new _.Wallet({
        vault
      });
      await wallet.unlock(null);
      await expect(wallet.getAddresses()).resolves.toEqual(expect.arrayContaining([account0.address])); // add a SimpleKeyring with the coming account in it

      const keyring = new _SimpleKeyring.SimpleKeyring();
      await keyring.addFromSeed((0, _util.hexToU8a)(account1.seed));
      await wallet.addKeyring(keyring); // call default keyring(hdkeyring)'s addAccount(), which will lead to a conflict with SimpleKeyring i.e:
      ...
      await wallet.addAccount(); // account1 can not be removed from hdkeyring, so it will be removed from SimpleKeyring
    ...
    it.skip('try resolve conflicts #2', async () => {
      ...
      const vault = '';
      const wallet = new _.Wallet({
        vault
      });
      ...
      await wallet.addKeyring(keyring);
      expect(wallet._accountKeyringMap[account0.address]).toBe(1); // generate account0 in default keyring and result to a conflict

      await wallet.addAccount(); // can not remove account0 from either keyring, so chose default keyring but keep it in another keyring as well

      expect(wallet._accountKeyringMap[account0.address]).toBe(0);
    ...
    it.skip('recover wallet from vault could fail without correct keyringTypes', async () => {
      const vault = '';
      const wallet = new _.Wallet({
        vault
      });
      await expect(wallet.unlock('test')).rejects.toThrow('keyring type SimpleKeyring not found');
      const wallet2 = new _.Wallet({
        vault,
        keyringTypes: [_SimpleKeyring.SimpleKeyring, _HDKeyring.HDKeyring]
      });
  ...
  describe('lock/unlock', () => {
    beforeEach(async () => {
      ...
      const keyring = new _SimpleKeyring.SimpleKeyring();
      [] = [await keyring.addPair(alice), await keyring.addPair(bob), await keyring.addFromSeed((0, _util.hexToU8a)(TEST_ACCOUNT.seed))];
      await wallet.addKeyring(keyring);
    });
